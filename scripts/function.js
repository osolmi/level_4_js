...
//v2. 이벤트addEventListener() 내에서 직접 사용자함수를 만드는 방식(반복안될때)
printBtn.addEventListener('click',function(){print();});

//v3. 내장함수에 함수명이 안들어가는데 fuction 키워드를 써야하는지 의문으로 생긴 새로운 함수
//화살표 함수 ()=>{}
printBtn.addEventListener('click',()=>{print();});
const num1 = Math.random();//0~1 랜덤한 숫자 생성 내장함수
const num2 = Math.floor(Math.random()*10);//소수점 제외해줌
...
//나무꾼은 금도끼를 잃어버렸습니다.
//나무꾼은 쇠도끼를 잃어버렸습니다.
function story2(name = '나무꾼', object = '쇠'){
    let person = name;
    let result = person + '은(는) ' + object +'도끼를 잃어버렸습니다.';
    console.log(result);
}
story2();//매개변수 아무것도 안적었을 때 모두 기본값
story2(undefined, '금'); //앞의 매개변수 정의 안돼서 기본값, 뒤 문자만 매개변수대입
story2('선녀');//앞 매개변수 문자대입, 뒤 매개변수 안적어서 기본값 처리
...
function cafeKiosk2(tem, me, num=1){
    ...
    return console.log(order); //콘솔 출력하고 끝내라
    console.log(order);
}
cafeKiosk2(0,0,1)//ICE 아메리카노 1잔 주문 완료
cafeKiosk2(1,3,2)//HOT 초코라떼 2잔 주문 완료
...
function japangi(drink, num=1){
    ...
    return console.log('잔액이 부족합니다.'); //리턴 뒤 콘솔싱행하고 함수 종료
    console.log(order);//리턴 아래 써있는 명령은 절대 실행 안됨
}
